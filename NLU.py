...
def tokenize_and_align_labels(examples):
    ...
    labels = []
    for i, word_ids in enumerate(tokenized_inputs.word_ids(batch_index=batch_idx) for batch_idx in range(len(examples["tokens"]))):
        label_ids = []
...
def evaluate_model():
    ...
    pred_labels = [[id2tag[pred] for pred, label in zip(sent_preds, sent_labels) if label != -100] for sent_preds, sent_labels in zip(predictions, labels)]
